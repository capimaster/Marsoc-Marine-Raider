...
for "_a" from 1 to _groupCount do 
{
	...
	for "_b" from 1 to _unitCount do 
	{
		switch _groupType do 
		{
			...
			case "GROUND":
			{
				_roadPos = getPos ([_spawnPos] call BIS_fnc_nearestRoad);
				_vehPos = [_roadPos, 0, 250, 10, 0, 0.8, 0,[],[_roadPos,[]]] call BIS_fnc_findSafePos;
	...
	switch _type do
	{
		...
		case "PATROL_ROADS":
		{
            ...
            for "_x" from 0 to 5 do {
                ...
                _currentWaypoint setWaypointCompletionRadius 30;
                if (_x == 5 ) then {
