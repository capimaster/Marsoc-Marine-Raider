...
while {_i < _waveCount} do
{
	...
	if (random 100 <= 25) then {_wave = [["SAD",10, "RED", _center, "AWARE", [""], [5, "CIRCLE_L"]], _minDistance, _maxDistance, 2, _groupName, _side, _faction, "GROUND", "STANDARD", "AAPLANE", selectRandom ["CAR","APC"], [1,2], ["COLUMN","LINE"], false] call CO_fnc_enemyAssault;} else {
	_wave = [["SAD",10, "RED", _center, "AWARE", [""], [5, "CIRCLE_L"]], _minDistance, _maxDistance, _waveSize, _groupName, _side, _faction, "MEN", ["STANDARD","RECON"], "AAPLANE", "CAR", [4,5,6],["STAG COLUMN","WEDGE","LINE","VEE"], false] call CO_fnc_enemyAssault;};
	_i = _i + 1;
